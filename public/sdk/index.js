...
function checkQRCodeStatus(qrcodeId) {
    let url = `/qrcode/check?qrcodeId=${qrcodeId}`
    return axios.get(url).then(res => {
        return res.data
    }).catch(err => {
        return err
...
function startPollingQRCodeStatus(options) {
    let { qrcodeId, interval = 1000, onPollingStart, onResult, onScanned, onExpired, onSuccess, onCancel, onError } = options
...
function startAppAuthScanning(options) {
    // 参数初始化
    let { mount = "authing__app-scan-qrcode-root-node", interval = 1000,
    ...
    const displayScannedUser = (nickname, photo) => {
        ...
        const shadow = document.createElement('div');
        shadow.style = `text-align:center;width: ${qrcodeSize.width}px;height: ${qrcodeSize.height}px;position: absolute;left: 50%;top: 0px;margin-left: -${qrcodeSize.width / 2}px;color:#fff;font-weight:600;display: flex;
        align-items: center; /*垂直居中*/
        justify-content: center; /*水平居中*/flex-direction: column-reverse;background-color: rgb(149 149 149 / 79%)`;
        shadow.id = shadowId;
        shadow.innerHTML = `<span style="margin-top: 10px">${nickname}正在登录中...</span>`;
